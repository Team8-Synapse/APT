...
import {
    ...
    BriefcaseBusiness, School, FileCheck, ClipboardCheck, UserPlus, Sparkles, Zap, Eye as EyeIcon, Edit, DownloadCloud, Linkedin
} from 'lucide-react';
...
import DriveApplicantsModal from '../../components/admin/DriveApplicantsModal';
...
const AdminDashboard = () => {
    ...
    const fetchApplicants = async (driveId) => {
        if (!driveId) return;
        try {
            console.log(`[Dashboard] Fetching applicants for driveId: ${driveId}`);
            const res = await axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5005/api'}/admin/drive/${driveId}/applications`);
            console.log(`[Dashboard] Fetched ${res.data.length} applicants`, res.data);
    ...
    const updateApplicationStatus = async (appId, status) => {
        try {
            await axios.patch(`${import.meta.env.VITE_API_URL || 'http://localhost:5005/api'}/admin/application/${appId}`, { status });
            // Refresh
            if (selectedDriveId) fetchApplicants(selectedDriveId);
            fetchStats();
        } catch (err) {
            console.error(err);
    ...
    const fetchDrives = async () => {
        try {
            ...
            setDrives(res.data);
            setLoading(false); // Ensure loading is off if this is the first fetch
        } catch (err) {
            console.error("Failed to fetch drives", err);
            // Fallback for demo if API fails
            if (stats.recentDrives) setDrives(stats.recentDrives);
        }
    };

    const deleteDrive = async (driveId) => {
        if (!window.confirm("Are you sure you want to delete this drive? This action cannot be undone and will delete all associated applications.")) return;
    ...
    useEffect(() => {
        ...
        if (driveIdFromUrl) {
            ...
            fetchApplicants(driveIdFromUrl);
        }
    }, [searchParams]);
    ...
    useEffect(() => {
        if (activeTab === 'drives' || activeTab === 'applications') {
            fetchDrives();
        }
    }, [activeTab]);
    ...
    const fetchAlumni = async () => {
        try {
            ...
            // setAlumni([]); // Don't use mock data, let it be empty to show DB state
        }
    };
    ...
    return (
        <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 page-enter">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 scroller-hide">
                ...
                {activeTab === 'applications' && (
                    <div className="lg:col-span-3 space-y-6 animate-fade-in-up">
                        <div className="flex justify-between items-center bg-white dark:bg-gray-800 p-4 rounded-2xl shadow-sm border border-gray-100">
                            <div>
                                <h2 className="text-2xl font-black" style={{ color: '#1f2937' }}>Application Tracker</h2>
                                <p className="text-gray-500 font-bold text-sm">Manage student applications and rounds</p>
                            </div>
                            <div className="flex items-center gap-4">
                                <select
                                    className="bg-gray-50 border border-gray-200 text-gray-900 text-sm rounded-xl focus:ring-amrita-maroon focus:border-amrita-maroon block p-3 font-bold min-w-[300px]"
                                    value={selectedDriveId}
                                    onChange={(e) => {
                                        setSearchParams({ tab: 'applications', driveId: e.target.value });
                                    }}
                                >
                                    <option value="">Select a Placement Drive</option>
                                    {stats.recentDrives?.map(drive => (
                                        <option key={drive._id} value={drive._id}>{drive.companyName} - {drive.jobProfile}</option>
                                    )) || []}
                                </select>
                            </div>
                        </div>
                            ...
                            <div className="flex justify-between items-center mb-6">
                                <div>
                                    <h3 className="text-lg font-black dark:text-white">
                                        {selectedDriveId ? 'Applicants List' : 'Select a Drive'}
                                    </h3>
                                    {selectedDriveId && (
                                        <p className="text-gray-500 font-bold">{applicants.length} students applied</p>
                                    )}
                                </div>
                                {selectedDriveId && (
                                    <button className="px-4 py-2 bg-green-100 text-green-700 rounded-lg font-bold text-xs hover:bg-green-200 flex items-center gap-2">
                                        <DownloadCloud size={16} /> Export List
                                    </button>
                                )}
                            </div>
                                    ...
                                    <table className="data-table">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>Current Stage</th>
                                                <th>Applied On</th>
                                                <th>Actions (Promote Rounds)</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {applicants.map((app, i) => (
                                                <tr key={i}>
                                                    <td className="font-black text-gray-900 dark:text-white">{app.studentId?.firstName} {app.studentId?.lastName}</td>
                                                    <td className="font-bold text-gray-500">{app.studentId?.rollNumber}</td>
                                                    <td className="font-bold">{app.studentId?.department}</td>
                                                    <td className="font-black" style={{ color: theme.maroon.primary }}>{app.studentId?.cgpa}</td>
                                                    <td>
                                                        <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${app.status === 'applied' ? 'bg-blue-100 text-blue-700' :
                                                    ...
                                                    <td className="flex gap-2">
                                                        {app.status !== 'rejected' && app.status !== 'offered' && (
                                                            <>
                                                                <button onClick={() => updateApplicationStatus(app._id, 'shortlisted')} className="px-3 py-1 bg-gray-100 hover:bg-gray-200 rounded text-xs font-bold" title="Shortlist">Shortlist</button>
                                                                <button onClick={() => updateApplicationStatus(app._id, 'round1')} className="px-3 py-1 bg-blue-50 hover:bg-blue-100 text-blue-600 rounded text-xs font-bold" title="Round 1">R1</button>
                                                                <button onClick={() => updateApplicationStatus(app._id, 'round2')} className="px-3 py-1 bg-blue-50 hover:bg-blue-100 text-blue-600 rounded text-xs font-bold" title="Round 2">R2</button>
                                                                <button onClick={() => updateApplicationStatus(app._id, 'hr_round')} className="px-3 py-1 bg-purple-50 hover:bg-purple-100 text-purple-600 rounded text-xs font-bold" title="HR Round">HR</button>
                                                                <button onClick={() => updateApplicationStatus(app._id, 'offered')} className="px-3 py-1 bg-green-50 hover:bg-green-100 text-green-600 rounded text-xs font-bold" title="Offer">Offer</button>
                                                            </>
                                                        )}
                                                        <button onClick={() => updateApplicationStatus(app._id, 'rejected')} className="p-1.5 bg-red-50 hover:bg-red-100 text-red-500 rounded tooltip" title="Reject">
                                                            <XCircle size={14} />
                                                        </button>
                                                    </td>
                                                </tr>
                                ...
                                <div className="text-center py-12">
                                    {selectedDriveId ? (
                                        <div className="flex flex-col items-center">
                                            ...
                                            <p className="text-gray-500 font-bold">No applications yet for this drive.</p>
                                            <p className="text-xs text-gray-400 mt-1">Students can apply via their dashboard.</p>
                                        </div>
                                    ) : (
                                        <div className="flex flex-col items-center">
                                            <Briefcase size={48} className="text-gray-200 mb-4" />
                                            <p className="text-gray-400 font-bold">Please select a drive from the dropdown above.</p>
                                        </div>
                                    )}
                                </div>
                            )}
                        ...
                        <div className="glass-card p-8">
                            <div className="flex justify-between items-center mb-8">
                                <div>
                                    ...
                                    <p className="text-sm font-bold text-gray-400 mt-1">{drives.length} total drives</p>
                                </div>
                                <button
                                    onClick={() => {
                                    ...
                                    style={{ backgroundColor: theme.maroon.primary }}
                                >
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                {drives.map((drive, i) => (
                                    <div key={i} className="p-6 bg-white dark:bg-gray-800 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-xl transition-all group relative overflow-hidden">
                                        <div className="absolute top-0 right-0 p-4">
                                            <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider ${drive.status === 'upcoming' ? 'bg-blue-50 text-blue-600' :
                                                drive.status === 'ongoing' ? 'bg-green-50 text-green-600' :
                                        ...
                                        <div className="flex items-center gap-4 mb-6">
                                            <CompanyLogo name={drive.companyName} size="md" className="rounded-2xl border border-gray-100" />
                                        </div>

                                        <h3 className="font-black text-xl text-gray-900 dark:text-white mb-1">{drive.companyName}</h3>
                                        <p className="text-xs font-bold text-gray-500 mb-3">{drive.jobProfile}</p>

                                        <span className="inline-block px-3 py-1 bg-purple-50 text-purple-600 rounded-lg text-[10px] font-black uppercase tracking-wider mb-6">
                                            {drive.jobType}
                                        </span>
                                        ...
                                        <div className="grid grid-cols-2 gap-y-4 gap-x-2 mb-6">
                                            <div>
                                                ...
                                                <p className="text-sm font-black text-gray-900 dark:text-white">
                                                    {new Date(drive.date).toLocaleDateString('en-US', { day: 'numeric', month: 'short' })}
                                                </p>
                                            </div>
                                            <div>
                                                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">CTC</p>
                                                <p className="text-sm font-black text-green-600">
                                                    ₹{(drive.ctcDetails?.ctc / 100000).toFixed(1)}L
                                                </p>
                                            </div>
                                            <div>
                                                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">Applicants</p>
                                                <p className="text-sm font-black text-gray-900 dark:text-white">{drive.registeredStudents?.length || 0}</p>
                                            </div>
                                            <div>
                                                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">Min CGPA</p>
                                                <p className="text-sm font-black text-gray-900 dark:text-white">{drive.eligibility?.minCgpa}</p>
                                            </div>
                                        </div>
                                        ...
                                        <div className="flex gap-2 pt-4 border-t border-gray-100 dark:border-gray-700">
                                            ...
                                            <button
                                                ...
                                                className="flex-1 py-2 bg-amber-50 text-amber-600 rounded-xl text-xs font-bold hover:bg-amber-100 transition-colors flex items-center justify-center gap-2"
                                            >
                                                <Edit size={14} /> Edit
                                            </button>
                                            <button
                ...
                {activeTab === 'alumni' && (
                    <div className="lg:col-span-3 animate-fade-in-up">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {alumni.map((alum, i) => (
                                <div key={i} className="glass-card p-6 hover:shadow-xl transition-all group flex flex-col items-center text-center relative">
                                    {/* Admin Actions */}
                                        ...
                                        <button
                                            onClick={async () => {
                                                if (window.confirm('Delete this alumni record?')) {
                                                    try {
                                                        await axios.delete(`${import.meta.env.VITE_API_URL || 'http://localhost:5005/api'}/alumni/member/${alum._id}`);
                                                        fetchAlumni();
                                                    } catch (e) {
                                                        console.error(e);
                                    ...
                                    <h3 className="text-xl font-bold text-gray-900 dark:text-white">{alum.name}</h3>
                                    <p className="text-sm font-medium text-amrita-maroon uppercase">{alum.role || 'Software Engineer'}</p>

                                    <div className="mt-4 w-full bg-gray-50 dark:bg-gray-800 rounded-xl p-3 border border-gray-100 dark:border-gray-700">
                                        <div className="flex items-center justify-center gap-2 text-gray-800 dark:text-gray-200 font-bold mb-1">
                                    ...
                                    <div className="flex gap-3 mt-6 w-full">
                                        ...
                                        <button className="p-2 bg-green-50 text-green-600 rounded-lg hover:bg-green-100" title="Message">
            ...
            <DriveApplicantsModal
                ...
                fetchApplications={() => fetchApplicants(selectedDriveId)}
            />
        </div>
    );
};
