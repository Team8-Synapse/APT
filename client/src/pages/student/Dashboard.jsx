...
const StudentDashboard = () => {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            // Robust userId retrieval - try context, then localStorage
            ...
            if (!currentUserId) {
                console.warn("No user ID found, skipping dashboard fetch");
                return;
            }

            try {
                const [dashboardRes, drivesRes, applicationsRes] = await Promise.all([
                    axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5005/api'}/student/dashboard-stats/${currentUserId}`),
                    axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5005/api'}/student/eligible-drives/${currentUserId}`),
                    axios.get(`${import.meta.env.VITE_API_URL || 'http://localhost:5005/api'}/applications/my-applications/${currentUserId}`)
                ]);
